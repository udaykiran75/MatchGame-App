...
import TabItemsList from '../TabItemsList'
...
class MatchGame extends Component {
  ...
  randomdlyDisplayImage = () => {
    const displayImage =
      imagesList[Math.ceil(Math.random() * imagesList.length)]
    this.setState({isActiveImage: displayImage})
  }

  onClickThambnail = id => {
    const {isActiveImage} = this.state
    if (isActiveImage.id === id) {
      this.setState(prevState => ({yourScore: prevState.yourScore + 1}))
      this.randomdlyDisplayImage()
    } else {
      this.onClearInterval()
      this.setState({istimerRunnig: false})
    }
  ...
  getTabIdListImages = isActiveTabId => {
    const filterTabIdList = imagesList.filter(
      eachItem => eachItem.category === isActiveTabId,
    )
    return filterTabIdList
  }
  ...
  render() {
    ...
    const activetabIdImageList = this.getTabIdListImages(isActiveTabId)
    const displayedImage = isActiveImage.imageUrl
    ...
    return (
      <div>
        <navbar className="nav-background">
          ...
          />
          <ul className="score-timer-div">
            <li>
              <p className="score-text">
                Score: <span className="span-elem">{yourScore}</span>
              </p>
            </li>
            <li className="score-timer-div">
              <img
                src="https://assets.ccbp.in/frontend/react-js/match-game-timer-img.png"
                alt="timer"
                className="timer-icon"
              />
              <p className="span-elem">{`${timeInSeconds} sec`}</p>
            </li>
          </ul>
        </navbar>
        {istimerRunnig && (
          <div className="bg-container">
            <img src={displayedImage} className="selected-image" alt="match" />
            <ul className="tabs-container">
              {tabsList.map(tabDetails => (
                <TabItems
                  tabDetails={tabDetails}
                  updateActiveTabId={this.updateActiveTabId}
                  isActive={isActiveTabId === tabDetails.tabId}
                  key={tabDetails.tabId}
                />
              ))}
            </ul>
            <ul className="images-list-container">
              {activetabIdImageList.map(eachItem => (
                <TabItemsList
                  imageDetails={eachItem}
                  key={eachItem.id}
                  onClickThambnail={this.onClickThambnail}
                />
              ))}
            </ul>
          </div>
        ...
        {!istimerRunnig && (
          <div className="score-card-container">
            <ScoreCard score={yourScore} restartGame={this.restartGame} />
          </div>
        )}
      </div>
    )
